...
WebUI.click(findTestObject('Object Repository/Nutritionist_Home/Unroll Client/Unroll_Client_Link'));

def pageContent = WebUI.getText(findTestObject('Object Repository/Body_Content'));
WebUI.delay(2);
if(pageContent.contains('Client disenrolled')){
	WebUI.closeBrowser();
} else {
	KeywordUtil.markFailed("Unroll failed");
}